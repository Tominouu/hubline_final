...
date_default_timezone_set('Europe/Paris'); // Définit le fuseau horaire pour tout le script

// Vérifier si l'utilisateur est connecté et gérer l'activité
if (isset($_SESSION['user_id'])) {
    $user_id = $_SESSION['user_id'];
    $current_time = (new DateTime())->format('Y-m-d H:i:s'); // Heure actuelle en Europe/Paris

    // Mettre à jour ou insérer l'activité de l'utilisateur
    $stmt = $conn->prepare("INSERT INTO active_users (user_id, last_activity) VALUES (?, ?) ON DUPLICATE KEY UPDATE last_activity = ?");
    $stmt->bind_param("iss", $user_id, $current_time, $current_time);
    $stmt->execute();
    $stmt->close();

    // Supprimer les utilisateurs inactifs
    $timeout = 5 * 60; // 5 minutes en secondes
    $cutoff_time = (new DateTime())->modify("-{$timeout} seconds")->format('Y-m-d H:i:s');
...
$query = "
    SELECT posts.id, posts.content, posts.created_at, users.id AS user_id, users.prenom, users.nom, users.profile_pic,
...
<body>
    <header>
        <a href="dashboard_test.php"><div class="logo">Hubline</div></a>
    ...
    <img src="uploads/67960b42c0073_Capture_d___cran_2025-01-24_164859.png" alt="Test Image">
    ...
    <main class="content">
        ...
        <aside class="sidebar">
            <p>Utilisateurs en ligne 🟢: <?php echo $active_users_count; ?></p>
        </aside>
    </main>
</body>
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        if (anchor) {
            ...
            if (target) {
                ...
                target.style.backgroundColor = "#fffae5";
